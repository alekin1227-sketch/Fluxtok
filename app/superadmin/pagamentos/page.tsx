...
export default async function PaymentsPage({ searchParams }: { searchParams: Promise<Record<string, string | undefined>> }) {
  ...
  const recurring = subscriptions.filter((s) => s.status === "ACTIVE" && isMercadoPagoCardProvider(s.provider));
  const annualRecurring = recurring.filter((s) => billingCycleFromProvider(s.provider) === "ANNUAL");
  ...
  const mrr = recurring.reduce((sum, s) => {
    const amount = Number(s.amount ?? PLAN_INFO[s.plan].price);
    return sum + (billingCycleFromProvider(s.provider) === "ANNUAL" ? amount / 12 : amount);
  }, 0);
  ...
  return <>
    ...
    <div className="admin-metric-grid admin-metric-grid-four">
      <div className="metric-card metric-success"><div className="metric-label">Cartões ativos</div><div className="metric-value">{recurring.length}</div><div className="metric-hint">MRR normalizado {money(mrr)} · {annualRecurring.length} anual(is)</div></div>
    ...
    <section className="section"><div className="section-bar"><div><h2>Assinaturas e acessos pagos</h2><p>O período gratuito não aparece quando a assinatura já está ativa.</p></div></div><div className="admin-table-wrap"><table className="admin-table"><thead><tr><th>Empresa</th><th>Plano</th><th>Status</th><th>Forma</th><th>Valor</th><th>Próximo marco</th></tr></thead><tbody>{filteredSubscriptions.map((s) => { const st = subscriptionStatus(s.status); const cycle = isMercadoPagoCardProvider(s.provider) ? billingCycleFromProvider(s.provider) : null; const paymentLabel = s.provider === "mercadopago_pix" ? "Pix" : cycle ? `Cartão ${billingCycleLabel(cycle).toLowerCase()}` : s.provider || "—"; const pendingCycle = s.pendingProvider && isMercadoPagoCardProvider(s.pendingProvider) ? billingCycleFromProvider(s.pendingProvider) : null; return <tr key={s.id}><td><strong>{s.company.name}</strong></td><td>{planLabel(s.plan)}{s.pendingPlan && <small className="table-sub">→ {planLabel(s.pendingPlan)}{pendingCycle ? ` · ${billingCycleLabel(pendingCycle)}` : ""} pendente</small>}</td><td><span className={`badge tone-${st.tone}`}>{st.label}</span></td><td>{paymentLabel}</td><td>{money(Number(s.amount ?? PLAN_INFO[s.plan].price))}{cycle === "ANNUAL" && <small className="table-sub">cobrança anual</small>}</td><td>{s.status === "TRIALING" ? `Teste até ${s.trialEndsAt.toLocaleDateString("pt-BR")}` : s.status === "ACTIVE" && s.provider === "mercadopago_pix" && s.currentPeriodEnd ? `Pago até ${s.currentPeriodEnd.toLocaleDateString("pt-BR")}` : s.status === "ACTIVE" && s.currentPeriodEnd ? `Próxima cobrança ${cycle === "ANNUAL" ? "anual " : ""}${s.currentPeriodEnd.toLocaleDateString("pt-BR")}` : s.status === "ACTIVE" ? "Recorrente" : "—"}</td></tr>; })}</tbody></table></div></section>
