...
export default async function PaymentsPage({ searchParams }: { searchParams: Promise<Record<string, string | undefined>> }) {
  ...
  return <>
    ...
    <div className="admin-metric-grid admin-metric-grid-four">
      ...
      <div className={`metric-card ${pendingPix ? "metric-warning" : ""}`}><div className="metric-label">Pix pendentes</div><div className="metric-value">{pendingPix}</div><div className="metric-hint">aguardando confirmação</div></div>
    </div>
    ...
    <section className="section"><div className="section-bar"><div><h2>Assinaturas e acessos pagos</h2><p>O período gratuito não aparece quando a assinatura já está ativa.</p></div></div><div className="admin-table-wrap"><table className="admin-table"><thead><tr><th>Empresa</th><th>Plano</th><th>Status</th><th>Forma</th><th>Valor</th><th>Próximo marco</th></tr></thead><tbody>{filteredSubscriptions.map((s) => { const st = subscriptionStatus(s.status); return <tr key={s.id}><td><strong>{s.company.name}</strong></td><td>{planLabel(s.plan)}</td><td><span className={`badge tone-${st.tone}`}>{st.label}</span></td><td>{s.provider === "mercadopago_pix" ? "Pix" : s.provider === "mercadopago" ? "Cartão" : s.provider || "—"}</td><td>{money(Number(s.amount ?? PLAN_INFO[s.plan].price))}</td><td>{s.status === "TRIALING" ? `Teste até ${s.trialEndsAt.toLocaleDateString("pt-BR")}` : s.status === "ACTIVE" && s.provider === "mercadopago_pix" && s.currentPeriodEnd ? `Pago até ${s.currentPeriodEnd.toLocaleDateString("pt-BR")}` : s.status === "ACTIVE" && s.currentPeriodEnd ? `Próxima cobrança ${s.currentPeriodEnd.toLocaleDateString("pt-BR")}` : s.status === "ACTIVE" ? "Recorrente" : "—"}</td></tr>; })}</tbody></table></div></section>
