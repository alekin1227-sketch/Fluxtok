...
export async function POST(req: NextRequest) {
  ...
  const planRaw = String(f.get("plan") || "STARTER") as BillingPlan;
  const plan = planRaw === BillingPlan.PRO ? BillingPlan.PRO : BillingPlan.STARTER;
  ...
  try {
    ...
    await audit({ companyId: user.companyId, userId: user.id, action: "BILLING_CHECKOUT_CREATED", entity: "subscription", metadata: { plan } });
