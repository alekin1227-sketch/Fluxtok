...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    await prisma.subscription.update({ where: { companyId: user.companyId }, data: { status: mapMercadoPagoStatus(remote.status), amount: remote.auto_recurring?.transaction_amount ? Number(remote.auto_recurring.transaction_amount) : local.amount, currentPeriodEnd: remote.next_payment_date ? new Date(remote.next_payment_date) : local.currentPeriodEnd } });
